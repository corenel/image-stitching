...
static void printUsage() {
  cout << "Rotation model images stitcher.\n\n"
          ...
          "Flags:\n"
          "  --preview\n"
          "      Run stitching in the preview mode. Works faster than usual "
          "mode,\n"
          "      but output image will have lower resolution.\n"
          "  --try_cuda (yes|no)\n"
          "      Try to use CUDA. The default value is 'no'. All default "
          "values\n"
          "      are for CPU mode.\n"
          ...
          "  --match_conf <float>\n"
          "      Confidence for feature matching step. The default is 0.65 for "
          "surf and 0.3 for orb.\n"
          "  --conf_thresh <float>\n"
          ...
          "  --ba_refine_mask (mask)\n"
          "      Set refinement mask for bundle adjustment. It looks like "
          "'x_xxx',\n"
          "      where 'x' means refine respective parameter and '_' means "
          "don't\n"
          "      refine one, and has the following format:\n"
          "      <fx><skew><ppx><aspect><ppy>. The default mask is 'xxxxx'. If "
          "bundle\n"
          "      adjustment doesn't support estimation of selected parameter "
          "then\n"
          "      the respective flag is ignored.\n"
          "  --wave_correct (no|horiz|vert)\n"
          "      Perform wave effect correction. The default is 'horiz'.\n"
          "  --save_graph <file_name>\n"
          "      Save matches graph represented in DOT language to <file_name> "
          "file.\n"
          "      Labels description: Nm is number of matches, Ni is number of "
          "inliers,\n"
          "      C is confidence.\n"
          "\nCompositing Flags:\n"
          "  --warp "
          "(affine|plane|cylindrical|spherical|fisheye|stereographic|"
          ...
          "      Number of filtering iterations of the exposure compensation "
          "gains.\n"
...
static int parseCmdArgs(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    if (string(argv[i]) == "--help" || string(argv[i]) == "/?") {
      ...
      if (string(argv[i + 1]) == "no" || string(argv[i + 1]) == "voronoi" ||
...
int main(int argc, char* argv[]) {
  ...
  //  cv::UMat matchMask(features.size(), features.size(), CV_8U,
  //  cv::Scalar(0)); for (int i = 0; i < num_images - 1; ++i) {
  ...
  LOGLN("Finding seams, time: " << ((getTickCount() - t) / getTickFrequency())
                                << " sec");
